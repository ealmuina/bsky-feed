...
func (q *Queries) GetUserDidsToRefreshStatistics(ctx context.Context, limit int32) ([]string, error) {
	rows, err := q.db.Query(ctx, getUserDidsToRefreshStatistics, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
